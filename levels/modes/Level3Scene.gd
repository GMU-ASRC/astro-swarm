...
func _walkthrough_lines() -> Array:
	return [
		...
		"6.  The GOAL TIME is when you reach the planet. Reach it and you win, and you can submit the run for a large XP payout.",
		"You can submit any run, detected or not - it is rendered on the website with your times either way.",
...
func _show_outcome(reason: String):
	...
	match reason:
		...
		_:
			...
			_phase_label.text = title
			_phase_label.add_theme_color_override("font_color", C_RED)
	_show_result(title, "%s\n\n%s\n\nSubmit the run either way - it is rendered on the website with your times." % [headline, _event_summary()], not _submitted)
