...
func _walkthrough_lines() -> Array:
	return [
		"GOAL: stop all %d waves before your line is spent." % MAX_WAVES,
		...
		"6.  There are %d waves, and the run ends early if the last defender is gone." % MAX_WAVES,
		"7.  The server benchmarks your algorithm over far longer runs than the %d waves you play here, and reports the share of evaders destroyed and the defenders lost." % MAX_WAVES,
		"Scroll to zoom, middle-drag to pan.",
	]

func _hint_lines() -> Array:
	return [
		"With %d defenders you get at most %d captures across the %d waves, so every body you spend badly is a wave you will not be there for." % [RING_COUNT, RING_COUNT, MAX_WAVES],
		...
		"The server keeps throwing waves long past the %d you see here, until your line is spent, so a trade you can afford now may not be one you can afford later." % MAX_WAVES,
